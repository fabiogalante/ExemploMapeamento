...
namespace Mapeamento
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var stopWatchImplicit = new Stopwatch();
            stopWatchImplicit.Start();
            MapImplicitOperator(funcionario);
            stopWatchImplicit.Stop();
            Console.WriteLine("Implicit operator: {0}", stopWatchImplicit.Elapsed);

            Console.WriteLine($"Diferença: {stopwatchAutoMapper.Elapsed - stopwatchMap.Elapsed}");
            Console.WriteLine($"Diferença: {stopwatchMap.Elapsed - stopWatchImplicit.Elapsed}");


            Console.ReadKey();
        }

        static FuncionarioViewModel MapImplicitOperator(Funcionario obj)
        {
            return obj;
        }
